...
open class DKAssetGroupDetailVC: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, DKGroupDataManagerObserver {
    ...
    public lazy var selectGroupButton: UIButton = {
		...
		button.titleLabel!.font = globalTitleFont ?? UIFont.boldSystemFont(ofSize: 17.0)
	...
	open func updateTitleView() {
        ...
        if self.imagePickerController.selectedAssets.count > 0 && self.imagePickerController.maxSelectableCount > 1 {
            self.selectGroupButton.setTitle("\(self.imagePickerController.selectedAssets.count) of \(self.imagePickerController.maxSelectableCount) Selected", for: .normal)
